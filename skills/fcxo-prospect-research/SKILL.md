...
value to this prospect – not a generic company dump.
...
2. **Why now, for your role** – 2-4 signals that connect to what the user does, each
   with the source.
3. **Likely pains** – framed as the buyer would feel them, not as expert categories.
...
If a workspace exists, save the brief to `leads/<Lead> - Lead.md` (update the existing
lead file if one exists - add the brief under a dated heading rather than overwriting)
with frontmatter `type: lead`, `status: new`, `updated: <date>`. Per the wiki convention in
